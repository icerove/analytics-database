...
const queryIsMine = (id, { req }) => {
  return pool.query(sql.checkQuery({ queryId: id, projectId: req.body.projectId})).then(res => {
...
const writeQueryValidator = [
  idIntSanitizer,
  param('id').custom(queryIsMine)
]
...
const createQuery = async (req, res) => {
  ...
  result = await pool.query(sql.createQuery({title, query, chartType, projectId}))
...
router.post('/', tokenRequired, createQueryValidator, validationErrorHandler, createQuery)
router.post('/:id', tokenRequired, writeQueryValidator, validationErrorHandler, updateQuery)
router.delete('/:id', tokenRequired, writeQueryValidator, validationErrorHandler, deleteQuery)
