...
const projectnameNotExistsValidator = 
  body('projectName')
  .trim()
...
const projectIsMine = (id, { req }) => {
  return pool.query(sql.checkProject({ userId: req.user.userid, projectId: id})).then(res => {
    if (res.rows.length == 0) {
...
const readQueryValidator = [
  idIntSanitizer,
  param('id').custom(projectExists)
]
...
router.post('/', tokenRequired, projectValidator, validationErrorHandler, createProject)
router.post('/:id', tokenRequired, writeProjectValidator, validationErrorHandler, updateProject)
router.delete('/:id', tokenRequired, writeProjectValidator, validationErrorHandler, deleteProject)
