...
const usernameNotExistsValidator =
  body('username')
    .trim()
...
const emailNotExistValidator =
  body('email')
    .trim()
    .custom(rejectEmailExists)

const checkUsernamePassword = (password, { req }) => {
  return pool.query(sql.checkPassword({username: req.body.username, password})).then(res => {
    if (res.rows.length == 0) {
...
const passwordCorrectValidator =
  body('password')
    .trim()
...
const usernameValidator =
  body('username')
    .trim()
    .matches(/^[\w\.\-_]+$/).withMessage("Can only include letters, numbers, '_', '-' and '.'")
...
const passwordValidator =
  body('password')
    .trim()
    .matches(/\d/).withMessage("Must contains a number")
    .matches(/[a-zA-Z]/).withMessage("Must contains a letter")
...
const signup = async (req, res) => {
  ...
  res.status(200).json({
    token: createToken({username: req.body.username, userid: result.rows[0].user_id}),
...
const login = (req, res) => {
  ...
  res.json({
    token: createToken({username: req.body.username, userid: req.user.user_id}),
    username: req.body.username
  })
}
